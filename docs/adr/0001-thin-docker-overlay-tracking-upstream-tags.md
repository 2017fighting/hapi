...
- The sync pushes with a `SYNC_TOKEN` PAT carrying `Workflows: write`. `GITHUB_TOKEN` can never update `.github/workflows/**` (`workflows` is not a grantable `permissions:` scope), so merging upstream workflow changes — `android-release.yml` arrived in v0.30.x — hard-fails the push until that secret exists. Rejected pushes are classified against the remote: only a remote that moved is a race and self-heals, anything else opens an `upstream-sync` issue instead of failing silently every day.
