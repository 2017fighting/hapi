...
export function registerSessionHandlers(socket: CliSocketWithData, deps: SessionHandlersDeps): void {
    ...
    socket.on('message', async (data: unknown) => {
        ...
        try {
            ...
            const msg = await store.messages.addMessage(sid, content, localId)
            ...
            console.error('session message handler failed', error)
        }
    })
    ...
    const handleUpdateMetadata: UpdateMetadataHandler = async (data, cb) => {
        ...
        try {
            ...
            console.error('update-metadata handler failed', error)
            cb({ result: 'error' })
        }
    }
    ...
    const handleUpdateState: UpdateStateHandler = async (data, cb) => {
        ...
        try {
            ...
            console.error('update-state handler failed', error)
            cb({ result: 'error' })
        }
    }
    ...
    socket.on('session-alive', async (data: SessionAlivePayload) => {
        ...
        try {
            ...
            console.error('session-alive handler failed', error)
        }
    })
    ...
    socket.on('session-ready', async (data: SessionReadyPayload) => {
        ...
        try {
            ...
            console.error('session-ready handler failed', error)
        }
    })
    ...
    socket.on('messages-consumed', async (data: { sid: string; localIds: string[]; clearQueuedThinkingGrace?: boolean }) => {
        ...
        try {
            ...
            try {
                await store.messages.markMessagesInvoked(data.sid, localIds, invokedAt)
                ...
                // Emit only after the DB write succeeds. Otherwise a transient
            ...
            console.error('messages-consumed handler failed', error)
        }
    })
    ...
    socket.on('session-end', async (data: SessionEndPayload) => {
        ...
        try {
            ...
            console.error('session-end handler failed', error)
        }
    })
}
