...
#
# hapi-hub — multi-arch Docker image of upstream (tiann/hapi) hub + web.
#
...
# (Shape mirrors hub/src/web/embeddedAssets.generated.d.ts.)
RUN printf '%s\n' \
...
# SQLite database lives at {HAPI_HOME}/hapi.db (/data/hapi.db) — mount a volume.
# DB_PATH overrides the location if needed.
...
# Auto-generated CLI_API_TOKEN, JWT/VAPID keys, settings.json, and the SQLite
# database (hapi.db) live here — mount a volume or state is lost on container
# recreation. Single-writer: run exactly one replica.
VOLUME ["/data"]
